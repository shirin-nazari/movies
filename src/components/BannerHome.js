                    ...
                    return (
                        <div key={data.id + 'bannerHome' + index} className='min-w-full min-h-[450px] lg:min-h-full overflow-hidden relative group transition-all' style={{ transform: `translateX(-${currentImage * 100}%)` }}>
                            <div className='w-full h-full'>
                                <img src={imageURL + data.backdrop_path} alt={data.title}
                            ...
                            <div className='container mx-auto'>
                                <div className="w-full absolute bottom-0 max-w-md px-3">
                                    ...
                                    <Link to={'/' + data?.media_type + '/' + data.id} className='bg-white px-4 py-2 text-black font-bold rounded mt-3 hover:bg-gradient-to-l from-red-700 to-orange-500 shadow-md transition-all hover:scale-105'>Play Now</Link>
                                </div>
                            </div>
                        </div>
                    )
